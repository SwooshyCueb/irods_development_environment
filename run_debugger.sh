...
if [ -n "$dry_run" ]; then  # -- print mount options for the debugger run
    ...
    if [ -e "${DEBUGGER_DOCKERFILE}" ]; then
        DOCKER_BUILDKIT=1 docker build --build-arg debugger_base=${base_image}  -f "${DEBUGGER_DOCKERFILE}" -t "${DEBUGGER_IMAGE}" . ${NO_CACHE}
        RUNNER_BASE="${DEBUGGER_IMAGE}"
    else
        RUNNER_BASE="${base_image}"
    fi
    if [ -n "$builder_build" ] || [ -n "$do_source_build" ]; then
        DOCKER_BUILDKIT=1 docker build -t "${BUILDER_IMAGE}" -f "${BUILDER_DOCKERFILE}" . ${NO_CACHE}
    fi
    ...
    if [ -e "${RUNNER_DOCKERFILE}" ]; then
        if [ -n "$runner_build" ]; then
            DOCKER_BUILDKIT=1 docker build --build-arg runner_base="${RUNNER_BASE}" -t "${RUNNER_IMAGE}" -f "${RUNNER_DOCKERFILE}" . ${NO_CACHE}
        fi
